...
def draw_env_record():
    ...
    for key in env_record.keys():
        ...
        # env_record[key].extend(move_average(np.load(f'history/0109_2/env/{key}_{episode}.npy', allow_pickle=True)))
...
def draw_training_record():
    ...
    # y = move_average(np.load(f'history/0109_2/loss_{episode}.npy'))
    x = np.arange(len(y))
