...
def draw():
    ...
    env_record = {'success_rate': [], 'reward': []}
    for key in env_record.keys():
        env_record[key].extend(move_average(np.load(f'record/env/{key}_{episode}.npy', allow_pickle=True)))
        # env_record[key].extend(move_average(np.load(f'history/0114_2/env/{key}_{episode}.npy', allow_pickle=True)))
    ...
    # y = move_average(np.load(f'history/0114_2/loss_{episode}.npy'))
    x = np.arange(len(y))
    plt.figure()
    plt.plot(x, y, 'r')
